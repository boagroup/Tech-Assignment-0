# Bartosz Solution 
# because some tasks don't specify a directory nor file i use $DIRECTORY and $FILE to make the solutions make sense
...
echo "speculate surprise relative regret evaluate" >> rain.txt && cp rain.txt rider.txt 
...
echo 'panel sister ditch panel needle' >> degree.txt && cp degree.txt support.txt
...
echo 'cater support shot panel excuse' >> recession.txt && cp recession.txt shallow.txt
...
mkdir regret && move regret $DIRECTORY 
